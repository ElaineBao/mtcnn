...
save_w = 256;
save_h = 160;
all_front = zeros(save_h,save_w,3,img_num,'uint8');
base_points = [];
errorlist = [];
...
for i=1:img_num
    ...
    error_flag = 0;
    if (w>h && h>64)
%         img_temp = imresize(img_temp,[save_h,save_w],'bilinear','antialiasing',false);
    ...
    elseif(w<h && w>64)
%         img_temp = imresize(img_temp,[save_w,save_h],'bilinear','antialiasing',false);
        img_temp = imresize(img_temp,[save_w,save_h]);
...
save('all_front256_160.mat','all_front','alllabel','errorids','errorlist');
