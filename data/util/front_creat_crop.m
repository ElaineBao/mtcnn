...
load('all_front256_160.mat')

pos_num = 5300;
root_path = '/ssd/zhaofan/IDCardDet_new1/data/front/';
save_path = '/ssd/zhaofan/IDCardDet_new1/data/traintest/front_crop1/';
save_size = 256;

load('all_front256_160.mat');

front_txt = fopen('front_train_crop1.txt','w');
...
for i=1:neg_num
    ...
    imwrite(neg_img,[save_path num2str(front_count) '.jpg']);
    offset = ([0 0; save_size 0; save_size save_size; 0 save_size] - label) / save_size;
    fprintf(front_txt,'%s %d %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n',[save_path num2str(front_count) '.jpg'],1,offset(:));
    ...
    if(1)
        ...
        offset = ([0 0; save_size 0; save_size save_size; 0 save_size] - label) / save_size;
        imwrite(neg_img,[save_path num2str(front_count) '_1.jpg']);
        fprintf(front_txt,'%s %d %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n',[save_path num2str(front_count) '_1.jpg'],1,offset(:));
    end
    ...
    if(1)
        ...
        offset = ([0 0; save_size 0; save_size save_size; 0 save_size] - label) / save_size;
        imwrite(neg_img,[save_path num2str(front_count) '_2.jpg']);
        fprintf(front_txt,'%s %d %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n',[save_path num2str(front_count) '_2.jpg'],1,offset(:));
    end
    ...
    if(1)
        ...
        label = (label_old - save_size / 2) * [0 -1;1,0] + save_size / 2;
        offset = ([0 0; save_size 0; save_size save_size; 0 save_size] - label) / save_size;
        imwrite(neg_img,[save_path num2str(front_count) '_3.jpg']);
        fprintf(front_txt,'%s %d %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n',[save_path num2str(front_count) '_3.jpg'],1,offset(:));
    end
