...
train_num = 5300;
test_num  = 163;
root_path = '/ssd/zhaofan/IDCardDet_new/data/front/';
save_path = '/ssd/zhaofan/IDCardDet_new/data/traintest/front/';
save_size = 256;

front_txt = fopen('front_train.txt','w');
...
for i=1:train_num
    ...
    img = imread([root_path all_front{i}]);
    [H,W,C] = size(img);
    if (1 == C)
    ...
    if(1)
        ...
        fprintf(front_txt,'%s %d %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n',[save_path num2str(i) '_1.jpg'],1,offset(:));
    end
...
%     % rotate 180
    if(1)
        ...
        fprintf(front_txt,'%s %d %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n',[save_path num2str(i) '_2.jpg'],1,offset(:));
    end
%     
%     % rotate 270
    if(1)
        ...
        fprintf(front_txt,'%s %d %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n',[save_path num2str(i) '_3.jpg'],1,offset(:));
    end
